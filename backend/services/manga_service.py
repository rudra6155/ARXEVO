...
# Free models to try in order of quality:
# black-forest-labs/FLUX.1-schnell (best free)
...
def generate_manga_cover(
    ...
    visual = archetype_visuals.get(
        archetype.lower(),
        "abstract personal power emanating outward"
    )
...
real person's life story."""
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            import time
            print("HF model loading, retrying in 20s...")
            time.sleep(20)
            retry = requests.post(
                MODEL_URL,
                headers=headers,
            ...
            if retry.status_code == 200:
                img_b64 = base64.b64encode(retry.content).decode('utf-8')
                return f"data:image/jpeg;base64,{img_b64}"
